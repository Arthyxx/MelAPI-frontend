...
export function Login() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Decodifica e guarda a role (opcional, mas útil)
      ...
      >
        <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-lg rounded-2xl shadow-2xl p-8 border border-amber-300 dark:border-amber-700 transition-all duration-300 hover:shadow-3xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label className="block text-amber-800 dark:text-amber-300 font-medium mb-1 text-sm">
                E-mail
              </label>
              <motion.div whileTap={{ scale: 0.99 }} variants={inputVariants}>
                <input
                  type="email"
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  className="w-full px-4 py-3 border border-amber-300 dark:border-amber-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white transition-all"
