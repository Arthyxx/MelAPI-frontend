...
export function Produtos() {
  ...
  const produtosExibidos = useMemo(() => {
    ...
    return produtos.filter((produto) => produto.stockQuantity > 0);
    ...
    const timeoutId = window.setTimeout(
      () => {
        const buscarProdutos = async () => {
          const requestId = ++latestProductsRequestIdRef.current;

          const isFirstLoad = !hasLoadedProductsRef.current;
      ...
      hasLoadedProductsRef.current ? 350 : 0,
    );

    return () => {
      window.clearTimeout(timeoutId);

      latestProductsRequestIdRef.current += 1;
    };
  }, [busca, ordenacao]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-100 dark:from-gray-950 dark:via-gray-900 dark:to-amber-950">
      ...
      <StoreFooter isLogged={isAuthenticated} />
    </div>
  );
}
