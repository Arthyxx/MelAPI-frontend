...
export function useClientesList() {
  ...
  const [roleFilter, setRoleFilter] = useState<ClienteRole | "">("");
  ...
  const totalClientesNaPagina = useMemo(() => {
    return clientes.filter((cliente) => cliente.role === "CLIENTE").length;
  }, [clientes]);

  const totalAtivosNaPagina = useMemo(() => {
    return clientes.filter((cliente) => cliente.active).length;
  }, [clientes]);
  ...
  const fetchClientes = useCallback(async () => {
    ...
    try {
      ...
      setError(getErrorMessage(axiosError, "Erro ao carregar clientes."));
    } finally {
      if (requestId === latestRequestIdRef.current) {
        setLoading(false);
      }
    }
  }, [activeFilter, limit, page, roleFilter, search]);
  ...
  const handleRoleFilterChange = (value: ClienteRole | "") => {
    setRoleFilter(value);
    setPage(1);
  };
