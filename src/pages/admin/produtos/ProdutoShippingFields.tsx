import type { ProdutoFormData } from "./produto.types";
...
export function ProdutoShippingFields({
  ...
  const shippingRequired = formData.active;
    ...
    <div className="lg:col-span-4">
      <div className="rounded-2xl border border-amber-100 bg-amber-50/60 p-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(event) =>
                onFieldChange("heightCm", event.target.value)
              }
              required={shippingRequired}
              className="h-12 w-full rounded-2xl border border-gray-200 bg-white px-4 font-medium outline-none transition focus:border-amber-400 focus:ring-4 focus:ring-amber-100"
            />
          ...
          <div>
            ...
            <input
              ...
              onChange={(event) => onFieldChange("widthCm", event.target.value)}
              required={shippingRequired}
              className="h-12 w-full rounded-2xl border border-gray-200 bg-white px-4 font-medium outline-none transition focus:border-amber-400 focus:ring-4 focus:ring-amber-100"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(event) =>
                onFieldChange("lengthCm", event.target.value)
              }
              required={shippingRequired}
              className="h-12 w-full rounded-2xl border border-gray-200 bg-white px-4 font-medium outline-none transition focus:border-amber-400 focus:ring-4 focus:ring-amber-100"
            />
        ...
        <p className="mt-4 text-xs font-semibold text-amber-800">
          {formData.active
            ? "Como o produto está ativo, peso, altura, largura e comprimento são obrigatórios."
            : "Produtos inativos podem ser salvos sem esses dados. Eles serão obrigatórios antes de ativar o produto."}
        </p>
      </div>
    </div>
  );
