...
export function useProdutosList() {
  ...
  const hasFilters = Boolean(
    search || categoryFilter || activeFilter || sort !== "id,asc",
  );
  ...
  const fetchProdutos = useCallback(async () => {
    ...
    try {
      ...
      setProdutos(Array.isArray(data.content) ? data.content : []);
      ...
      setError(getErrorMessage(axiosError, "Erro ao carregar produtos."));
    } finally {
      if (requestId === latestRequestIdRef.current) {
        setLoading(false);
      }
    }
  }, [activeFilter, categoryFilter, limit, page, search, sort]);
