...
describe("AuthContext", () => {
  ...
  it("deve restaurar uma sessão válida salva no localStorage", () => {
    ...
    expect(result.current.user?.id).toBe(25);
  ...
  it("deve encerrar automaticamente a sessão quando o token expirar", () => {
    vi.useFakeTimers();
  ...
  it("deve sincronizar login realizado em outra aba", () => {
    const token = createTestToken({
      sub: 50,
      email: "cliente50@email.com",
    });
    ...
    expect(result.current.user?.id).toBe(50);

    expect(result.current.token).toBe(token);
  });

  it("deve sincronizar logout realizado em outra aba", () => {
    const token = createTestToken({
      sub: 60,
    });
